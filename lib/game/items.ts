...
import gradeMapJson from "./item-grade.generated.json";
...
// Grade for NON-equipment items (materials, special). Unlike gear, these don't
// encode their grade in the item key — it lives in the game's ItemInfoData GRADE
// column (e.g. 190001 "หินวิญญาณ - ปกติ" = IMMORTAL/red). Generated by
// agent/build_item_grades.py; any key absent here defaults to 0 (COMMON).
const GRADE_MAP = gradeMapJson as Record<string, number>;
...
/** Grade (0..9) of an item. Equipment (class 3-6) encodes it in the key digits;
 * materials/special items carry it in static data (GRADE_MAP), not the key. */
export function itemGrade(itemKey: number): number {
  const cls = Math.floor(itemKey / 100000);
  if (String(itemKey).length === 6 && cls >= 3 && cls <= 6) {
    return decodeItemKey(itemKey).grade;
  }
  return GRADE_MAP[String(itemKey)] ?? 0;
}
